...
function BatchGeneratePanel() {
  ...
  // Reference images & fixed prompt
  const [referenceImages, setReferenceImages] = useState([]) // [{url, name, localUrl}]
  ...
  async function handleRefImageChange(e) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const resp = await fetch('/api/batch/upload-ref-image', { method: 'POST', body: form })
    ...
    try {
      const resp = await fetch('/api/batch/start', {
        ...
        body: JSON.stringify({
          ...
          defaultAspectRatio,
          defaultImageSize,
          referenceImages: referenceImages.map((r) => ({ url: r.url, name: r.name })),
    ...
    <div className="batch-generate-panel">
      ...
      {step === 'preview' && (
        <div className="batch-section">
          ...
          {/* Reference images & fixed prompt */}
          <div className="batch-ref-prompt-section">
            <div className="batch-ref-images">
              <div className="batch-ref-images-header">
                <label>参考图（可选）</label>
                <button
              ...
              {referenceImages.length > 0 ? (
                <div className="batch-ref-images-list">
                  {referenceImages.map((img, i) => (
                    <div key={i} className="batch-ref-image-item">
                      <img src={img.url} alt={img.name} />
                      <button
                        className="batch-ref-image-remove"
                        onClick={() => removeReferenceImage(i)}
                        title="移除"
                      >
                        <Icon.X size={10} />
                      </button>
                      <span className="batch-ref-image-name">{img.name}</span>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="batch-hint" style={{ margin: '4px 0' }}>暂未添加参考图，可上传产品/风格参考图，所有生成将以此为基准</p>
              )}
            </div>

            <div className="batch-fixed-prompt">
              <label>固定 Prompt（可选）</label>
              <textarea
                className="input-field"
                rows={3}
                placeholder="例如：真实质感、4K、光线统一、相机拍摄...&#10;将自动追加到每条提示词前面"
                value={fixedPrompt}
                onChange={(e) => setFixedPrompt(e.target.value)}
              />
              <p className="batch-hint" style={{ margin: '4px 0' }}>将自动追加到每条提示词的前面，用于统一风格/质量要求</p>
            </div>
          </div>
