...
const SYSTEM_PROMPT = `你是一个专业的 AI 营销文案专家，同时也能处理设计任务。

你的写作原则：
1. 严格基于知识库：只使用知识库文档中的真实信息，绝不编造数据、规格、数字
2. 知识库不足时诚实说明，不要凭空想象填充内容
3. 理解受众：用户说的"专业"指的是可信、有深度，不是枯燥的技术手册
4. 文案要有血有肉：用自然的口吻，说人话，让读者产生兴趣
5. 标题要有吸引力但不能假大空，拒绝"震撼来袭""颠覆行业"这类套话

写作风格：
- 像资深销售写给客户看的邮件/文章，专业但不死板
- 突出产品的真实优势，用知识库中的实际参数说话
- 没有数据的部分，用产品理念和应用场景来补充
- 段落自然过渡，像和朋友介绍产品一样流畅
...
产品型号匹配：
- 严格按用户指定的型号来写，不混淆同系列其他型号
- 知识库文档可能是英文名（如 R5-Curve = 任意弧，R5-Straight = 直角锁）`;
...
3. 如果需要图片，数量（最多5张）和比例（1:1/16:9/9:16等）
...
典型示例：
"帮我生成一张R5直角锁质感图，实拍质感，加上公司logo" → needsText: false, needsImages: true, imageCount: 1
"给我一段BPro产品1000字英文推广文案" → needsText: true, needsImages: false, taskType: "marketing_copy", language: "en"
"帮我写篇R5任意弧介绍文章，配一张图" → needsText: true, needsImages: true, imageCount: 1
...
async function handleChatStream({ conversationId, userMessage, emit, signal }) {
  ...
  if (needsText) {
    ...
    try {
      const textResult = await llmService.complete(llmConfig, SYSTEM_PROMPT,
        `## 任务类型\n请生成一篇${langLabel}${typeLabel}。\n\n## 用户需求\n${userMessage}\n${historySection}\n## 知识库内容（注意：请只使用与用户指定产品型号精确匹配的内容）\n${knowledgeResult.texts.join('\n\n') || '（无相关知识库内容）'}${contextSection}\n\n⚠️ 对话连续性：如果对话历史显示你之前生成过内容，而用户现在要求修改/微调（如"语气更专业"、"加一段"、"缩短"），请修改你上次生成的内容而不是重写一篇全新的。\n\n⚠️ 编造=失败：只能使用上方知识库中出现的真实数据和信息。如果知识库缺少某些参数或细节，用产品理念、应用场景、给客户带来的价值来描述，绝不凭空编造数字。\n\n⚠️ 格式：纯文本，禁止Markdown，标题【】，可直接复制发布。\n\n请直接输出生成的内容，不要添加额外的说明。`);
      generatedText = textResult.content;
