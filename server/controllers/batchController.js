...
// POST /api/batch/upload-ref-image — 上传单张参考图
async function handleUploadRefImage(req, res, next) {
  ...
  try {
    ...
    // 获取本地 URL（相对路径，供前端预览）
    const localUrl = `/uploads/${path.basename(tmpPath)}`;

    // 上传到公共图床（让 GRSai API 能访问）
    const publicUrl = await uploadLocalImageToPublicUrl(localUrl);

    logger.info('参考图上传成功', { localUrl, publicUrl });

    res.json({
      success: true,
      data: { url: publicUrl, localUrl, name: file.originalname },
    });
  } catch (err) {
    next(err);
  } finally {
...
async function handleBatchStart(req, res, next) {
  try {
    ...
    const job = await BatchJob.create({
      ...
      fixedPrompt: String(fixedPrompt || '').trim(),
      status: 'queued',
    });
...
async function processBatchJob(jobId) {
  ...
  try {
    ...
    for (const item of job.items) {
      ...
      // 拼接最终 prompt：固定 Prompt（如果有） + 原始 prompt
      const basePrompt = job.fixedPrompt ? `${job.fixedPrompt.trim()}，` : '';
      const finalPrompt = basePrompt + item.prompt;
      ...
      try {
        ...
        logger.info('批量生成：调用 API', { jobId, index: item.index, model, hasRef: job.referenceImages?.length > 0, hasFixedPrompt: !!job.fixedPrompt });
