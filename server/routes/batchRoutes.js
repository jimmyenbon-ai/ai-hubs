...
// 参考图上传（允许图片格式，限制 5MB）
const uploadRefImage = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, cb) => cb(null, tmpDir),
    filename: (_req, file, cb) => {
      const ext = path.extname(file.originalname || '.png');
      cb(null, `ref-img-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${ext}`);
    },
  }),
...
router.post('/upload-ref-image', uploadRefImage.single('file'), handleUploadRefImage);
